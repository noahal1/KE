/**
 * Body diagram: anatomically accurate front/back figures built from the
 * polygon model of `react-body-highlighter` (MIT), remapped to the app's
 * muscle vocabulary and re-styled in the app's single-ink aesthetic
 * (fill opacity = relative training volume, same scale as the bar list).
 *
 * Geometry notes:
 *  - Each region is 1..n <polygon>s in the 1000×2000 model space; polygons
 *    are copied verbatim from the upstream model (mirrored halves ship as
 *    separate polygons — no runtime mirroring needed).
 *  - Upstream has no separate lats region, so its "upper back" polygons are
 *    split at y≈560 into upper-back (upper-inner) and lats (lower-outer).
 *  - The anterior model's lower-leg polygons are the shin area → mapped to
 *    `tibialis`; posterior lower legs (+ soleus) stay with `calves`.
 *  - Knees and hip notches have no muscle key; they render as silhouette
 *    filler (SILHOUETTE_FILL) so the figure stays continuous.
 *  - `core` has no region of its own; its bar-list entry still shows in the
 *    adjacent list, the figure simply leaves it unpainted.
 */
...
/** One anatomical region: 1..n polygons ("x y x y ...", 1000×2000 space). */
interface Region {
  key: string; // app muscle vocabulary
  view: "front" | "back";
  polys: string[];
}
...
const REGIONS: Region[] = [
  ...
  },
  {
    key: "chest",
    view: "front",
    polys: ["518 416 510 551 580 580 678 555 706 473 620 416", "298 465 314 555 408 580 482 551 478 420 376 420"],
  },
  {
    key: "shoulders",
    view: "front",
    polys: ["784 531 796 478 792 412 759 380 710 363 722 429 714 473", "282 473 212 531 200 478 204 408 245 371 286 371 269 433"],
  },
  {
    key: "biceps",
    view: "front",
    polys: ["167 682 180 714 229 661 290 539 278 494 204 559", "714 494 702 547 763 661 816 718 829 690 788 555"],
  },
  {
    key: "triceps",
    view: "front",
    polys: ["694 555 694 616 759 727 776 702 755 673", "224 694 298 555 298 608 229 731"],
  },
  {
    key: "abs",
    view: "front",
  ...
  {
    // Anterior lower legs = shin area (tibialis anterior).
    key: "tibialis",
    view: "front",
    polys: [
  ...
  // ---------------- back (posterior) ----------------
  {
    key: "neck",
    view: "back",
    polys: ["506 0 460 9 409 55 404 128 451 200 557 200 591 136 596 47 557 13"],
  },
  {
    key: "traps",
    view: "back",
  ...
  },
  {
    key: "shoulders",
    view: "back",
    polys: ["294 370 230 391 174 443 183 536 243 494 272 464", "711 370 783 396 826 447 817 536 749 489 723 451"],
  },
  {
    // Upper (inner) half of the upstream "upper back" polygons.
    key: "upper-back",
    view: "back",
    polys: ["311 387 281 489 285 553 366 540 336 413", "689 387 719 494 715 562 634 545 664 417"],
  },
  {
    // Lower (outer) half of the upstream "upper back" polygons.
    key: "lats",
    view: "back",
    polys: ["285 553 340 753 472 711 472 664 366 540", "715 562 660 753 528 711 528 664 634 545"],
  },
  {
    key: "triceps",
    view: "back",
  ...
  },
  {
    key: "lower-back",
    view: "back",
    polys: ["477 728 345 770 353 834 494 1021 468 830", "523 728 655 770 647 834 506 1021 532 838"],
  },
  {
    key: "glutes",
    view: "back",
  ...
  },
  {
    ...
    polys: [
      ...
      // Soleus (part of the calf complex).
...
/** Non-muscle silhouette fillers (knees, hip notches) so the figure stays
 *  continuous. Order: front fillers first, then back. */
const SILHOUETTE_FILL: Record<"front" | "back", string[]> = {
  front: [
    ...
    // Hip notches between obliques and inner-thigh polygons.
    "339 784 347 939 396 922 392 837",
    "657 788 649 943 600 927 604 833",
  ],
  back: [
...
export interface BodyDiagramValue {
  /** 0..1 relative volume (caller normalizes). */
  intensity: number;
  /** Preformatted tooltip detail (e.g. "12,340 kg"). */
  detail: string;
}
...
function Figure({
  ...
  view: "front" | "back";
  values: Map<string, BodyDiagramValue>;
}) {
  const { t } = useTranslation();
  const [hovered, setHovered] = useState<string | null>(null);
  const regions = REGIONS.filter((r) => r.view === view);
  return (
    <svg viewBox="0 0 1000 2000" className="h-72 w-auto" role="img" aria-label={t(`muscle.${view}`)}>
      {/* silhouette fillers (no data, no tooltip) */}
      {SILHOUETTE_FILL[view].map((points, i) => (
        <polygon key={i} points={points} fill={INK} fillOpacity={0.06} />
      ))}
      {/* muscle fills */}
      {regions.map((r) => {
        const v = values.get(r.key);
        const opacity = v ? 0.08 + 0.87 * clamp01(v.intensity) : 0.06;
        const label = t(`muscle.${r.key}`);
        const title = v ? `${label} · ${v.detail}` : label;
        ...
        return (
          ...
          >
            <title>{title}</title>
            {r.polys.map((points, i) => (
              <polygon
                key={i}
                points={points}
                fill={INK}
                fillOpacity={opacity}
                stroke={isHovered ? INK : "none"}
      ...
      {/* etched region borders on top (hairline anatomy-chart look) */}
      {regions.flatMap((r) =>
        r.polys.map((points, i) => (
          <polygon
            key={`${r.key}-${i}`}
            ...
            fill="none"
            stroke={INK}
            strokeOpacity={hovered === r.key ? 0.45 : 0.2}
            strokeWidth={4}
          />
        )),
      )}
...
/** Front + back figure pair with caption labels. */
    ...
    <div className="flex justify-center gap-6">
      {(["front", "back"] as const).map((view) => (
        <div key={view} className="flex flex-col items-center gap-2">
          <Figure view={view} values={values} />
          <span className="ed-label">{t(`muscle.${view}`)}</span>
        </div>
      ))}
    </div>
  );
